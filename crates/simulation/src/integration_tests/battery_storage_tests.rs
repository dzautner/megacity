...
use crate::battery_storage::{BatteryState, BatteryTier, BatteryUnit, ROUND_TRIP_EFFICIENCY};
...
#[test]
fn test_battery_charges_when_excess_capacity() {
    let mut city = new_baseline_city();

    // 200 MW capacity, 100 MW demand => 100 MW surplus capacity
...
fn test_battery_discharge_applies_efficiency() {
    // Test the unit-level discharge efficiency directly.
    let mut unit = BatteryUnit::new(BatteryTier::Large, 0, 0);
    unit.stored_mwh = 100.0; // Full

    // Request 40 MWh of output
    let discharged = unit.discharge(40.0);

    // Energy drawn from storage should be 40/0.85 = ~47.06 MWh
    let drawn = 100.0 - unit.stored_mwh;
    let efficiency = discharged / drawn;
    assert!(
        (efficiency - ROUND_TRIP_EFFICIENCY).abs() < 0.01,
        "Efficiency should be ~85%, got {:.1}%",
        efficiency * 100.0
    );
    assert!(
        (discharged - 40.0).abs() < 0.1,
        "Should deliver ~40 MWh, got {}",
        discharged
    );
}
...
fn test_battery_respects_reserve_threshold() {
    ...
    // Large deficit: 10 MW capacity but 100 MW demand
    city.world_mut().spawn(make_plant(10.0, 0.0));
    spawn_demand(&mut city, 100.0);

    // Battery at 25% SOC (25 MWh of 100 MWh capacity)
    // Reserve is 20% = 20 MWh, so only 5 MWh available for draw
...
fn test_battery_charge_clamped_to_rate() {
    ...
    // Massive surplus: 500 MW capacity, 10 MW demand
    city.world_mut().spawn(make_plant(500.0, 30.0));
    spawn_demand(&mut city, 10.0);
...
#[test]
fn test_multiple_batteries_charge() {
    let mut city = new_baseline_city();

    // 200 MW capacity, 50 MW demand => 150 MW surplus
    city.world_mut().spawn(make_plant(200.0, 0.0));
    spawn_demand(&mut city, 50.0);
...
fn test_battery_soc_tracking() {
    ...
    // Surplus capacity for charging: 200 MW capacity, 50 MW demand
    city.world_mut().spawn(make_plant(200.0, 0.0));
    spawn_demand(&mut city, 50.0);
