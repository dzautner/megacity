...
fn test_fire_truck_dispatched_to_fire_via_road() {
    let mut city = TestCity::new()
        ...
        .with_building(30, 49, ZoneType::Industrial, 1)
        .rebuild_csr();

    ignite_building(&mut city, 30, 49, 60.0);

    // Run enough ticks for multiple dispatch cycles (every 10 ticks)
    city.tick(30);
...
fn test_closer_station_dispatches_with_shorter_path() {
    // Far station at (10,50), close station at (25,50), fire at (30,49)
    // Place stations OFF the road so they don't interfere with road cells
    let mut city = TestCity::new()
        .with_road(10, 50, 40, 50, RoadType::Local)
        .with_service(10, 49, ServiceType::FireStation)
        .with_service(25, 49, ServiceType::FireStation)
        .with_building(40, 49, ZoneType::Industrial, 1)
        .rebuild_csr();

    ignite_building(&mut city, 40, 49, 60.0);
    ...
    assert!(
        !fire_vehicles.is_empty(),
        "Should have dispatched a fire truck (total_dispatches={}, vehicles={})",
        state.total_dispatches,
        state.vehicles.len()
    );

    // The vehicle should originate from the closer station (25,49)
    let vehicle = &fire_vehicles[0];
    assert_eq!(
        vehicle.origin,
        (25, 49),
        "Should dispatch from closer station at (25,49), but dispatched from {:?}",
        vehicle.origin
    );
}
...
fn test_no_dispatch_without_road_path() {
    ...
    ignite_building(&mut city, 50, 49, 60.0);
    ...
    let fire_to_far: Vec<_> = state
        .vehicles
        .iter()
        .filter(|v| v.kind == EmergencyKind::Fire && v.target == (50, 49))
        .collect();
...
fn test_vehicle_capacity_limits_dispatches() {
    ...
    ignite_building(&mut city, 30, 49, 80.0);
    ignite_building(&mut city, 40, 49, 80.0);
    ignite_building(&mut city, 50, 49, 80.0);
...
fn test_ambulance_dispatched_to_severe_fire() {
    let mut city = TestCity::new()
        .with_road(10, 50, 40, 50, RoadType::Local)
        .with_service(10, 49, ServiceType::Hospital)
        .with_service(12, 49, ServiceType::FireStation)
        .with_building(40, 49, ZoneType::Industrial, 1)
        .rebuild_csr();

    // Severe fire triggers ambulance dispatch (threshold 30.0)
    ignite_building(&mut city, 40, 49, 80.0);
    ...
    assert!(
        ...
        "Should dispatch ambulance to severe fire (intensity >= 30). \
         total_dispatches={}, vehicles={:?}",
        state.total_dispatches,
        state.vehicles.iter().map(|v| format!("{:?}", v.kind)).collect::<Vec<_>>()
    );
}
...
fn test_vehicles_advance_along_path() {
    ...
    if !has_vehicle {
        // Run more ticks for dispatch cycle
        city.tick(20);
    }
    ...
    let initial_index = {
        ...
        if state.vehicles.is_empty() {
            return; // No path found, skip test gracefully
        }
