...
fn test_invariant_tel_aviv_employment_drift_corrected() {
    ...
    // After correction, most buildings should have occupants >= actual workers.
    // Some small drift is expected because job_seek may assign workers between
    // the validator pass and the end of the slow-tick cycle.
    let world = city.world_mut();
    ...
    // Allow up to 10% of job buildings to still have minor drift
    // (job_seek can add workers between validator pass and cycle end)
    let max_drift = (total_job_buildings as f32 * 0.10).ceil() as u32 + 1;
    assert!(
        drift_buildings <= max_drift,
        "After correction, most buildings should have occupants >= workers. \
         Drift buildings: {drift_buildings}/{total_job_buildings} (max allowed: {max_drift})"
    );
}
...
#[test]
fn test_invariant_no_overcapacity_on_truly_empty_city() {
    use crate::simulation_invariants::InvariantViolations;
    use crate::test_harness::TestCity;

    // No buildings at all: the validator should find zero overcapacity.
    let mut city = TestCity::new();
    ...
    assert_eq!(
        ...
        "No job overcapacity violations expected on truly empty city"
    );
}
