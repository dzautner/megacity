...
fn test_road_maintenance_initial_condition_synced() {
    ...
    // degrade_roads runs every 50 ticks and syncs on first run
    city.tick(50);
    ...
    for x in 10..=30 {
        if grid.get(x, 10).cell_type == CellType::Road {
            ...
            // After sync (200) minus base degradation (0*2+1=1) + repair (5 if < 200)
            assert!(
                condition >= 190,
...
fn test_road_repair_cap_at_200() {
    ...
    if grid.get(15, 10).cell_type == CellType::Road {
        ...
        // u8 can't exceed 255; verify condition is reasonable after repair+degrade
        assert!(
            cond > 0,
