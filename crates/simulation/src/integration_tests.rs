...
fn test_policy_cost_deducted_from_budget_after_tax_collection() {
    ...
    // Run enough ticks for a tax collection cycle
    city.tick_slow_cycles(3);
    let treasury_after = city.budget().treasury;
    // Treasury should have decreased (policy costs are expenses)
    // We cannot assert exact amounts because other income/expenses also apply,
    // but the expense breakdown should show policy costs.
