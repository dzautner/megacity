...
fn test_road_maintenance_initial_condition_synced() {
    ...
    // Check a cell that should be a road
    let grid = city.grid();
    let mut found_road = false;
    for x in 10..=30 {
...
/// Test that road condition degrades with traffic volume.
/// Higher traffic should cause more degradation.
#[test]
fn test_road_condition_degrades_with_traffic() {
    use crate::road_maintenance::RoadConditionGrid;
    use crate::traffic::TrafficGrid;
    ...
    // Manually set traffic levels on road cells
    {
        let world = city.world_mut();
        let mut traffic = world.resource_mut::<TrafficGrid>();
        // High traffic on cell (15, 10)
        traffic.set(15, 10, 10);
        // Low traffic on cell (25, 10)
        traffic.set(25, 10, 0);
    }

    // Record conditions before degradation
    let cond_high_before = city.resource::<RoadConditionGrid>().get(15, 10);
    let cond_low_before = city.resource::<RoadConditionGrid>().get(25, 10);

    // Run another 50 ticks for degradation to run again
    city.tick(50);

    let cond = city.resource::<RoadConditionGrid>();
    let cond_high_after = cond.get(15, 10);
    let cond_low_after = cond.get(25, 10);

    // The cell with high traffic should have degraded more
    let degradation_high = cond_high_before.saturating_sub(cond_high_after);
    let degradation_low = cond_low_before.saturating_sub(cond_low_after);

    assert!(
        degradation_high > degradation_low,
        "High-traffic road should degrade more ({degradation_high}) than low-traffic road ({degradation_low})"
    );
}

/// Test that degradation formula matches: degradation = (traffic_level * 2 + 1).min(255)
#[test]
fn test_road_degradation_formula_with_known_traffic() {
    use crate::road_maintenance::RoadConditionGrid;
    use crate::traffic::TrafficGrid;
    ...
    // Set known traffic level
    let traffic_level: u16 = 7;
    {
        let world = city.world_mut();
        let mut traffic = world.resource_mut::<TrafficGrid>();
        traffic.set(15, 10, traffic_level);
    }

    let grid = city.grid();
    // Only test if the cell is actually a road (segments may rasterize differently)
    if grid.get(15, 10).cell_type == CellType::Road {
        ...
        // Expected degradation from traffic: 7*2+1 = 15
        // But repair also runs, so actual loss = degradation - repair
        // With budget_level=1.0, repair_amount = 5 (for cells < 200)
        // If condition was already < 200, repair adds 5, so net = 15 - 5 = 10
        // If condition was >= 200, no repair, so net = 15
        let expected_degradation: u8 = (traffic_level * 2 + 1).min(255) as u8;
        let actual_drop = cond_before.saturating_sub(cond_after);

        // The degradation component should be present; repair may offset some
        // With default budget, repair = 5 for condition < 200
        if cond_before < 200 {
            // Net = degradation - repair = 15 - 5 = 10
            assert_eq!(
                actual_drop,
                expected_degradation.saturating_sub(5),
                "Net degradation should be traffic degradation minus repair (condition was {cond_before})"
            );
        } else {
            // At or above 200, no repair is applied
            assert_eq!(
                actual_drop, expected_degradation,
                "Degradation should equal traffic formula when condition >= 200"
            );
        }
    }
}
...
/// Test that repair does not exceed 200 threshold (repair only applies when condition < 200).
#[test]
fn test_road_repair_cap_at_200() {
    ...
    if grid.get(15, 10).cell_type == CellType::Road {
        ...
        // Just verify it's reasonable (repair applied then degradation applied)
...
/// Test that sustained heavy traffic eventually degrades roads below 100.
#[test]
fn test_road_sustained_heavy_traffic_degrades_below_threshold() {
    use crate::road_maintenance::{RoadConditionGrid, RoadMaintenanceBudget};
    use crate::traffic::TrafficGrid;

    let mut city = TestCity::new().with_road(10, 10, 20, 10, RoadType::Local);
    city.tick(50); // sync conditions
    ...
    // Apply heavy traffic repeatedly
    for _ in 0..10 {
        {
            let world = city.world_mut();
            let mut traffic = world.resource_mut::<TrafficGrid>();
            traffic.set(15, 10, 20); // heavy traffic: degradation = 20*2+1 = 41 per cycle
        }
        city.tick(50);
    }
    ...
    if grid.get(15, 10).cell_type == CellType::Road {
        ...
        assert!(
            cond < 100,
            "Sustained heavy traffic with no budget should degrade road below 100, got {cond}"
        );
    }
}
...
fn test_road_speed_factor_boundary_at_100() {
    let cond = city_with_condition_at(15, 10, 100);
    assert_eq!(
...
fn test_road_speed_factor_boundary_at_25() {
    let cond = city_with_condition_at(15, 10, 25);
    assert_eq!(
        cond.road_condition_speed_factor(15, 10),
        0.7,
...
fn test_road_speed_factor_boundary_at_99() {
    let cond = city_with_condition_at(15, 10, 99);
    assert_eq!(
        cond.road_condition_speed_factor(15, 10),
        0.7,
...
fn test_road_speed_factor_boundary_at_24() {
    let cond = city_with_condition_at(15, 10, 24);
    assert_eq!(
        cond.road_condition_speed_factor(15, 10),
        0.0,
