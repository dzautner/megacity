...
fn test_game_params_defaults_match_original_constants() {
    ...
    // Road defaults match grid.rs hardcoded values
...
fn test_game_params_saveable_roundtrip() {
    ...
    assert!(
        (restored.economy.starting_treasury - 99_999.0).abs() < f64::EPSILON,
        "roundtrip should preserve starting_treasury"
    );
    assert!(
        (restored.citizen.speed - 200.0).abs() < f32::EPSILON,
        "roundtrip should preserve citizen speed"
    );
}

#[test]
fn test_game_params_modifying_construction_ticks() {
    // Verify that changing construction_ticks in GameParams affects how long
    // buildings take to construct.
    ...
    // Set very short construction time
    {
        let world = city.world_mut();
    ...
    // Zone some cells for residential
    let mut city = city
        .with_zone_rect(11, 9, 19, 9, ZoneType::ResidentialLow)
        .with_zone_rect(11, 11, 19, 11, ZoneType::ResidentialLow);

    // Tick enough for buildings to spawn and construction to complete
    city.tick(20);

    // With construction_ticks=2, buildings should be done constructing quickly
    let world = city.world_mut();
    ...
    // We should have some buildings, and most should be done constructing
    // (with 2-tick construction, after 20 ticks they should all be done)
    if buildings > 0 {
        assert!(
            still_constructing < buildings,
            "With 2-tick construction time, most of {} buildings should be done, but {} still constructing",
            buildings,
            still_constructing
        );
    }
...
fn test_game_params_zone_demand_bootstrap() {
    ...
    // Run a slow cycle to trigger demand computation
    city.tick_slow_cycle();

    let demand = city.resource::<ZoneDemand>();
    // With roads but no buildings and high bootstrap demand, residential demand
    // should be meaningfully positive
    assert!(
        demand.residential > 0.0,
        "With bootstrap_demand=0.9 and roads present, residential demand should be positive, got {}",
        demand.residential
    );
}
...
fn test_game_params_road_params_lookup() {
    ...
    // Verify all road types can be looked up
    let road_types = [
        RoadType::Local,
        RoadType::Avenue,
        RoadType::Boulevard,
