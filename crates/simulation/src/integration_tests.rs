...
fn test_district_aggregate_population_matches_residential_occupants() {
    ...
    // Place two residential buildings in distinct statistical districts.
    let bld_a = (5, 5); // district (0,0)
    let bld_b = (DISTRICT_SIZE + 3, 5); // district (1,0)

    let mut city = TestCity::new()
        .with_building(bld_a.0, bld_a.1, ZoneType::ResidentialLow, 1)
        .with_building(bld_b.0, bld_b.1, ZoneType::ResidentialLow, 1);

    // Run a full slow cycle so aggregate_districts fires.
    city.tick_slow_cycle();

    // Read actual building occupants after simulation ran.
    let mut occ_a = 0u32;
    let mut occ_b = 0u32;
    {
        let world = city.world_mut();
...
/// Verify per-district job counts (commercial, industrial, office) after aggregation.
/// Reads actual building state post-simulation to validate consistency.
#[test]
fn test_district_aggregate_job_counts() {
    ...
    // Read actual building state to compute expected values.
...
/// Verify that per-district average happiness is correctly computed from
/// citizens living in that district. Reads actual citizen happiness after
/// the slow cycle to verify the district average.
#[test]
fn test_district_aggregate_happiness_average() {
    ...
    // Read actual citizen happiness in the target district.
    ...
    let mut count = 0u32;
    {
        ...
        for (details, home) in query.iter(world) {
            let (cdx, cdy) = Districts::district_for_grid(home.grid_x, home.grid_y);
            if cdx == dx && cdy == dy {
    ...
    if count > 0 {
        ...
        assert!(
            ...
            "Expected avg happiness ~{expected}, got {avg}"
        );
    }
}

/// Verify that the sum of all statistical district populations equals the
/// total statistical population returned by `total_statistical_population()`.
#[test]
fn test_district_population_sums_to_total() {
    ...
    // Sum individual district populations
...
/// Verify that the `DistrictMap` correctly tracks which cells belong to
/// which player-defined district and that reassignment works.
#[test]
fn test_district_map_cell_assignment_tracking() {
...

    {
        ...
        // Assign a block of cells to district 0 ("Downtown")
        for x in 10..14 {
            for y in 10..14 {
        ...
        // Reassign some cells to district 1 ("Suburbs")
        for x in 12..14 {
            for y in 12..14 {
...
/// Verify the `district_stats` system computes per-player-district population
/// from buildings whose cells are assigned to that district.
#[test]
fn test_district_stats_population_from_buildings() {
    ...
    // Assign building cells to player districts
    {
        let world = city.world_mut();
        let mut dmap = world.resource_mut::<DistrictMap>();
        dmap.assign_cell_to_district(bld_a.0, bld_a.1, 0); // Downtown
        dmap.assign_cell_to_district(bld_b.0, bld_b.1, 1); // Suburbs
    }
...
/// Verify that player-district crime stats are computed from the CrimeGrid.
#[test]
fn test_district_stats_crime_average() {
    ...
    // Assign a 2x2 block to district 0 and set crime values
    {
        let world = city.world_mut();
        let mut dmap = world.resource_mut::<DistrictMap>();
        for x in 50..52 {
    ...
    let dmap = city.resource::<DistrictMap>();
    // Expected average crime: (40 + 60 + 80 + 20) / 4 = 50.0
...
/// Verify that player-district happiness is averaged from citizens whose
/// home cell is assigned to that district. Reads actual citizen state
/// post-simulation to verify consistency.
#[test]
fn test_district_stats_happiness_from_citizens() {
    ...
    // Assign the home cell to district 2
    {
        let world = city.world_mut();
        let mut dmap = world.resource_mut::<DistrictMap>();
    ...
    // Read actual citizen happiness assigned to district 2
    let mut happiness_sum = 0.0f32;
    let mut count = 0u32;
    {
...
/// Verify that districts with no buildings or citizens have zero stats.
#[test]
fn test_district_empty_district_has_zero_stats() {
    ...
    // Assign some cells to district 0 but place no buildings there
    {
        let world = city.world_mut();
        let mut dmap = world.resource_mut::<DistrictMap>();
        for x in 100..105 {
    ...
    // Automatic statistical districts for this region
    let districts = city.resource::<Districts>();
...
/// Verify that residential capacity is correctly aggregated per district.
#[test]
fn test_district_aggregate_residential_capacity() {
    use crate::districts::Districts;

    let pos_a = (5, 5); // district (0,0)
    let pos_b = (6, 5); // same district (0,0)
...
/// Verify that buildings in different statistical districts do not bleed
/// their stats into neighboring districts. Reads actual building state
/// post-simulation to verify district isolation.
#[test]
fn test_district_no_stat_bleed_between_districts() {
    use crate::districts::{Districts, DISTRICT_SIZE};

    // Place a residential building in district (0,0) and an industrial
    // building in district (1,0), right at the district boundary.
    let pos_a = (DISTRICT_SIZE - 1, 0); // last column of district (0,0)
    let pos_b = (DISTRICT_SIZE, 0); // first column of district (1,0)
    ...
    // District (1,0) should have industrial capacity but no residential capacity
    assert!(
    ...
    // District (0,0) should have residential capacity but no industrial capacity
    assert!(
...
/// Verify that removing a cell from a player district correctly updates
/// the district's cell set and the cell_map.
#[test]
...
/// Verify the default DistrictMap comes pre-populated with the expected
/// number of named districts.
#[test]
