...
/// Sample terrain elevation at a world XZ position.
...
/// `y_offset` is added to the sampled terrain Y at each point.
...
pub(crate) fn emit_dashed_line(
    ...
    let mut dash_start: Option<(Vec2, Vec2, f32)> = None;
    ...
    for i in 0..=total_steps {
        ...
        if should_dash && !in_dash {
            dash_start = Some((left, right, y));
            in_dash = true;
        } else if in_dash && (!should_dash || i == total_steps) {
            if let Some((sl, sr, sy)) = dash_start {
...
/// `y_offset` is added to the sampled terrain Y at each vertex.
#[allow(clippy::too_many_arguments)]
